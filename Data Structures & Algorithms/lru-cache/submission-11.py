...
class LRUCache:
    ...
    def put(self, key: int, value: int) -> None:
        if key not in self.nodeMap and len(self.nodeMap)==self.cap: #this doesnt always work if it already exists 
            #pop last one
            print(f"this is the head {self.head.key}")
            print(f"this is the head {self.head.nextV.key}")
            print(f"this is the head {self.head.nextV.nextV}")
            #the head is ddisconeencted? 

            key1=(self.head.nextV).key
            print(f"current nodemap: {self.nodeMap}")
            print(f"key1 is {key1} althoguh why would i remove")
